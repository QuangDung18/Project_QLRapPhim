    ...
    public IActionResult Update(Phim updatedPhim)
    {
      if (!ModelState.IsValid)
      {
        ...
        return RedirectToAction("Index", new { id = updatedPhim.PhimId });
			...
			var existingPhimWithSameName = _context.Phims
			  .FirstOrDefault(p => p.TenPhim == updatedPhim.TenPhim && p.PhimId != updatedPhim.PhimId);

			if (existingPhimWithSameName != null)
			{
				TempData["Error"] = "Phim đã tồn tại!";
				return RedirectToAction("Index", new { id = updatedPhim.PhimId });
			}
